...
- **`tests/NOrganizze.Tests/`** — Integration-style tests against the live API (`NOrganizze.Tests.csproj`, xUnit v3).
- **`tools/ApiValidator/`** — Validates models and endpoints against the real API; generates empirical OpenAPI and reports.
- **`tools/NOrganizze.Mcp/`** — MCP server (Docker-distributed, stdio transport) that exposes client operations as MCP tools.
- **`organizze-api-openapi.yaml`** — OpenAPI reference at repo root.
- **`README.md`** — Short overview; points to `USAGE.md`.
...
- **Namespaces**: One folder/namespace per domain (e.g. `NOrganizze.Accounts`, `NOrganizze.Transactions`). Option types for create/update/list live beside their service (e.g. `TransactionListOptions`, `AccountCreateOptions`).
...
Build the library project alone:
...
Pack the library (version often passed in CI from git tag):

```bash
dotnet pack src/NOrganizze/NOrganizze.csproj -c Release
```

**ApiValidator** (hits the real Organizze API; reads credentials from **user secrets**, same as tests):
...
Set from the **`tests/NOrganizze.Tests`** or **`tools/ApiValidator`** directory (either works; same store):

```bash
dotnet user-secrets set "Organizze:Email" "you@example.com"
...
- Outputs under `tools/ApiValidator/output/`: reports (`api-validation-report.md`, `.json`) and empirical spec `organizze-api-openapi-empirical.yaml`.
- See `tools/ApiValidator/README.md` for full behavior and git-tracking notes for `output/`.

## CI/CD

- **`.github/workflows/publish-nuget.yml`**: On push of tags matching `v[0-9]+.[0-9]+.[0-9]+*` (or manual `workflow_dispatch`), restores, builds Release, packs `src/NOrganizze/NOrganizze.csproj` with `PackageVersion` from the tag, pushes to NuGet.org via OIDC (`NuGet/login` + `NUGET_USER` secret).
...
- Prefer extending **`Service`** and **`NOrganizzeClient`** rather than ad-hoc HTTP calls from multiple places.
...
- Enforce readonly behavior server-side: mutating tools (`create`, `update`, `delete`) must always pass through the readonly guard before calling client services.
- `readonly` defaults to `true`; do not weaken this default unless explicitly requested.
- Credentials/config precedence must remain: `.mcp-norganizze.json` first, then `NORGANIZZE_*` environment variables.
- Never log or echo secrets (`apiKey`, auth headers, full credential payloads) in tool outputs or logs.
- Keep Docker execution path healthy: `tools/NOrganizze.Mcp/Dockerfile` should continue to build and run with the target runtime expected by the project.
- Docker Hub canonical repository for MCP image is `docker.io/graduenz/norganizze-mcp`; keep docs and examples aligned with this exact name.
- Preserve Docker tag/version parity with release tags: `vX.Y.Z` -> image `X.Y.Z` (+ `latest` for stable only); prerelease tags must not move `latest`.
- When MCP behavior changes, update both `tools/NOrganizze.Mcp/README.md` and root `README.md` MCP notes to keep setup instructions accurate.
- Validate MCP changes with at least:
  - `dotnet build`
...
- This means date-cursor-based pagination works: you can advance `start_date` to a specific day within a month and the API will only return transactions from that day onward.
